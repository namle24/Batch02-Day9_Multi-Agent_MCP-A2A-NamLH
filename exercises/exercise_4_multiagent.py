...
def check_routing(state: State) -> dict:
    ...
    return {
        ...
        "needs_compliance": any(
            kw in question_lower for kw in ["compliance", "sec", "regulation"]
        ),
        "needs_privacy": any(
            kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "rò rỉ"]
        ),
    }


def route_to_agents(state: State) -> list[Send]:
    """Dispatch các specialist agents cần thiết bằng LangGraph Send API."""
    tasks = []
...
biện pháp khắc phục và rủi ro xử phạt.
Trả lời dưới 120 từ."""
...
Hãy tạo một báo cáo ngắn gọn, có cấu trúc rõ ràng, dưới 250 từ."""
